...
function setup(){
  ...
  //cria solo invisível
  soloInvisivel = createSprite(windowWidth / 2,windowHeight - 20,windowWidth,40);
  soloInvisivel.visible = false;
  
  //cria sprite do T-Rex
  trex = createSprite(80,110,20,50);
  trex.scale = 0.1;
  ...
  fimDoJogo = createSprite(windowWidth / 2,windowHeight / 2 - 30);
  fimDoJogo.addImage(imagemFimDoJogo);

  reiniciar = createSprite(windowWidth / 2,windowHeight / 2 + 30);
...
function draw(){
  ...
  if(estadoJogo === JOGAR){
    ...
    if(keyDown('space') && trex.y>windowHeight - 120 || touches.length > 0 && trex.y>windowHeight-120){
      trex.velocityY = -15; 
      somSalto.play();
      touches = [];
    }
...
function gerarObstaculos(){
  ...
  if(frameCount %60 === 0){
    obstaculo = createSprite(windowWidth,windowHeight - 90 ,10,40);
